...
type RefItem = { id: number; name: string; code?: string | null; description?: string | null };
...
const LabTestTypes: React.FC = () => {
  ...
  const load = async () => {
    ...
    setError(null);
    try {
      const res = await axios.get<RefItem[]>(`${API_URL}/lab-tests/refs/test-types`).catch(() => ({ data: [] as RefItem[] }));
  ...
  const save = async (e: React.FormEvent) => {
    ...
    try {
      if (editing) {
        await axios.put(`${API_URL}/lab-tests/refs/test-types/${editing.id}`, {
          name: n,
          code: code.trim() || null,
          description: description.trim() || null,
        });
      } else {
        await axios.post(`${API_URL}/lab-tests/refs/test-types`, {
          name: n,
          code: code.trim() || null,
          description: description.trim() || null,
        });
  ...
  const remove = async (id: number) => {
    ...
    try {
      await axios.delete(`${API_URL}/lab-tests/refs/test-types/${id}`);
      await load();
    } catch (err) {
      console.error(err);
    ...
    <>
      <div className="pageHead">
        <div>
          ...
          <div className="h1">Вид испытания</div>
          <div className="muted mini">Справочник видов лабораторных испытаний.</div>
        </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', zIndex: 1000, padding: '20px', overflowY: 'auto' }}>
          <div className="card" style={{ maxWidth: 520, width: '100%', margin: '20px 0' }}>
            <div className="cardHead">
              <div className="title">{editing ? 'Редактировать' : 'Добавить'} вид испытания</div>
            ...
            <div className="cardBody">
              <form onSubmit={save}>
                ...
                <div className="field">
                  <label>Код</label>
                  <input value={code} onChange={(e) => setCode(e.target.value)} placeholder="например: concrete_strength" />
                </div>
                <div className="field">
