...
class ProjectChange(Base):
    ...
    change_type = Column(Enum(ChangeType, native_enum=False), nullable=False, comment="Тип изменения")
    ...
    status = Column(Enum(ChangeStatus, native_enum=False), default=ChangeStatus.DRAFT, comment="Статус")
...
class ChangeApproval(Base):
    ...
    status = Column(Enum(ApprovalStatus, native_enum=False), default=ApprovalStatus.PENDING, comment="Статус согласования")
